        ...
        try {
          switch (action) {
            case 'getUserAdditionInfo':
              customer = await models.Customers.getWidgetCustomer({ email: data.user.email, phone: data.user.phoneNumber });
              ...
              filter = {}
              dealIdsOfCustomer = await models.Conformities.savedConformity({ mainType: 'customer', mainTypeId: customer._id, relTypes: ['deal'] });

              if (data.carId) {
                const dealIdsOfCar = await models.Conformities.savedConformity({ mainType: 'car', mainTypeId: data.carId, relTypes: ['deal'] });
                filter['_id'] = { $in: dealIdsOfCar }
              }

              const wStageIds = await models.Stages.find({ probability: 'Won' }, { _id: 1 })
              filter['stageId'] = { $in: wStageIds }

              const dealCount = await models.Deals.find({ $and: [{ _id: { $in: dealIdsOfCustomer } }, filter] }).countDocuments();

              return sendSuccess({ loyalty, dealCount });

            case 'createCar':
              try {
                car = await checkModels(models, models.Cars, 'createCar', {...data});
                ...
                await models.Conformities.addConformity({ mainType: 'customer', mainTypeId: customer._id, relType: 'car', relTypeId: car._id });
              } catch (e) {
            ...
            case 'filterCars':
              ...
              const carIds = await models.Conformities.savedConformity({ mainType: 'customer', mainTypeId: customer._id, relTypes: ['car'] })
              ...
              return sendSuccess(await models.Cars.aggregate([
            ...
            case 'getCar':
              ...
              return sendSuccess({ car, category: await models.CarCategories.findOne({ _id: car.categoryId }) });
            ...
            case 'filterCarCategories':
              ...
              return sendSuccess(await models.CarCategories.aggregate([
                { $match: filter },
                {
                  $lookup: {
                    from: 'car_categories',
            ...
            case 'getProduct':
              const product = await models.Products.findOne({ _id: data.productId });

              if (!product) {
                return sendError('Product not found')
              }
              return sendSuccess({ product, category: await models.ProductCategories.findOne({ _id: product.categoryId }) });
            ...
            case 'filterProductCategories':
              ...
              return sendSuccess(await models.ProductCategories.aggregate(
                [
                  { $match: filter },
                  {
                    $lookup: {
            ...
            case 'filterProducts':
              ...
              return sendSuccess(await models.Products.aggregate([
                { $match: filter },
                {
                  $lookup: {
                    from: 'product_categories',
            ...
            case 'filterDeals':
              ...
              const wonStageIds = await models.Stages.find(stageFilter, { _id: 1 })
              filter['stageId'] = { $in: wonStageIds }

              const deals = await models.Deals.find({ $and: [{ _id: { $in: dealIdsOfCustomer } }, filter] });

              const stageIds = deals.map(deal => deal.stageId);
              const stages = await models.Stages.find({ _id: { $in: stageIds } }, { _id: 1, name: 1, pipelineId: 1 });
              const pipelineIds = stages.map(stage => stage.pipelineId);
              const pipelines = await models.Pipelines.find({ _id: { $in: pipelineIds } }, { _id: 1, name: 1, boardId: 1 });
              const boardIds = pipelines.map(pipeline => pipeline.boardId);
              const boards = await models.Boards.find({ _id: { $in: boardIds } }, { _id: 1, name: 1 });
            ...
            case 'createDeal':
              ...
              await models.Conformities.addConformity({ mainType: 'deal', mainTypeId: deal._id, relType: 'customer', relTypeId: customer._id })

              if (data.carId) {
                await models.Conformities.addConformity({ mainType: 'deal', mainTypeId: deal._id, relType: 'car', relTypeId: data.carId })
              } else {
                const carIds = await models.Conformities.savedConformity({ mainType: 'customer', mainTypeId: customer._id, relTypes: ['car'] });
                if (carIds.length > 0) {
                  await models.Conformities.addConformity({ mainType: 'deal', mainTypeId: deal._id, relType: 'car', relTypeId: carIds[0] })
                }
              }
            ...
            case 'getKnowledgeBaseTopicDetail':
              return sendSuccess(await models.KnowledgeBaseTopics.getTopic(data._id));

            case 'getKnowlegeBaseForCatIds':
              return sendSuccess({
                topics: await models.KnowledgeBaseTopics.find({ categoryIds: { $in: data.categoryIds } }),
                categories: await models.KnowledgeBaseCategories.find({ _id: { $in: data.categoryIds } })
              });

            case 'filterKnowledgeBaseCategories':
              const topic = await models.KnowledgeBaseTopics.getTopic(data.topicId);
            ...
            case 'getKnowledgeBaseCategory':
              return sendSuccess(await models.KnowledgeBaseCategories.getCategory(data));

            case 'filterKnowledgeBaseArticles':
              const category = await models.KnowledgeBaseCategories.getCategory(data.categoryId);

              const articles = await models.KnowledgeBaseArticles.find({
                _id: { $in: category.articleIds }, status: 'publish'
              }).sort({
                createdAt: -1,
              });
            ...
            case 'getKnowledgeBaseArticle':
              return sendSuccess(await models.KnowledgeBaseArticles.getArticle(data._id));
          }
        } catch (e) {
          sendError(e.message)
        }
      ...
      handler: async (msg, { models }) => {
        ...
        switch (action) {
          case 'createCustomer':
            ...
            customer = customer
              ? await models.Customers.updateMessengerCustomer({ _id: customer._id, doc })
          ...
          case 'updateCar':
            return sendSuccess(await models.Cars.updateCar(data._id, { ...data }));
