...
var enhance = compose(withPropsOnChange(['format'], function (_ref) {
    ...
    if (!format || _.isFunction(format)) return { format: format };
 ...
 */
var memoizedColorModifier = _.memoize(function (method, _amount) {
    var amount = parseFloat(_amount);

    return _.memoize(function (d) {
...
var quantizeColorScales = {
    ...
    // Diverging
    BrBG: _.last(d3ScaleChromatic.schemeBrBG),
    PRGn: _.last(d3ScaleChromatic.schemePRGn),
    PiYG: _.last(d3ScaleChromatic.schemePiYG),
    PuOr: _.last(d3ScaleChromatic.schemePuOr),
    RdBu: _.last(d3ScaleChromatic.schemeRdBu),
    RdGy: _.last(d3ScaleChromatic.schemeRdGy),
    RdYlBu: _.last(d3ScaleChromatic.schemeRdYlBu),
    RdYlGn: _.last(d3ScaleChromatic.schemeRdYlGn),
    spectral: _.last(d3ScaleChromatic.schemeSpectral),

    // Sequential (Single Hue)
    blues: _.last(d3ScaleChromatic.schemeBlues),
    greens: _.last(d3ScaleChromatic.schemeGreens),
    greys: _.last(d3ScaleChromatic.schemeGreys),
    oranges: _.last(d3ScaleChromatic.schemeOranges),
    purples: _.last(d3ScaleChromatic.schemePurples),
    reds: _.last(d3ScaleChromatic.schemeReds),

    // Sequential (Multi-Hue)
    BuGn: _.last(d3ScaleChromatic.schemeBuGn),
    BuPu: _.last(d3ScaleChromatic.schemeBuPu),
    GnBu: _.last(d3ScaleChromatic.schemeGnBu),
    OrRd: _.last(d3ScaleChromatic.schemeOrRd),
    PuBuGn: _.last(d3ScaleChromatic.schemePuBuGn),
    PuBu: _.last(d3ScaleChromatic.schemePuBu),
    PuRd: _.last(d3ScaleChromatic.schemePuRd),
    RdPu: _.last(d3ScaleChromatic.schemeRdPu),
    YlGnBu: _.last(d3ScaleChromatic.schemeYlGnBu),
    YlGn: _.last(d3ScaleChromatic.schemeYlGn),
    YlOrBr: _.last(d3ScaleChromatic.schemeYlOrBr),
    YlOrRd: _.last(d3ScaleChromatic.schemeYlOrRd)
};
...
var guessQuantizeColorScale = function guessQuantizeColorScale(colors) {
    ...
    // user defined colors
    if (_.isArray(colors)) return d3Scale.scaleQuantize().range(colors);
...
var getColorRange = function getColorRange(instruction) {
    ...
    if (___default.isFunction(instruction)) return instruction;

    if (ordinalColorScales[instruction]) return ordinalColorScales[instruction];

    if (___default.isArray(instruction)) return d3Scale.scaleOrdinal(instruction);
...
var getColorsGenerator = function getColorsGenerator(colors, colorBy) {
    ...
    if (___default.isFunction(colorBy)) return colorBy;

    var scale = void 0;
    var getColorId = function getColorId(d) {
        return ___default.get(d, colorBy);
    };
    ...
    } else if (___default.isArray(colors)) {
...
var withDimensions = (function () {
    return compose(defaultProps({
        ...
        return props.width !== nextProps.width || props.height !== nextProps.height || !_.isEqual(props.margin, nextProps.margin);
    }, function (props) {
...
var getLabelGenerator = function getLabelGenerator(_label, labelFormat) {
    var getRawLabel = ___default.isFunction(_label) ? _label : function (d) {
        return ___default.get(d, _label);
    };
    var formatter = void 0;
    if (labelFormat) {
        formatter = ___default.isFunction(labelFormat) ? labelFormat : d3Format.format(labelFormat);
    }
...
var getAccessorFor = function getAccessorFor(directive) {
    return ___default.isFunction(directive) ? directive : function (d) {
        return d[directive];
    };
};
...
var withMotion = (function () {
    return compose(setPropTypes(motionPropTypes), defaultProps({
        ...
        return {
            boundSpring: _.partialRight(reactMotion.spring, {
...
var withTheme = (function () {
    ...
    return compose(setPropTypes((_setPropTypes = {}, _setPropTypes[srcKey] = PropTypes.object, _setPropTypes)), withPropsOnChange([srcKey], function (props) {
        ...
        return _ref2 = {}, _ref2[destKey] = _.merge({}, defaultTheme, props[srcKey]), _ref2;
    }));
});
...
var Axes$1 = shouldUpdate(function (props, nextProps) {
    return props.xScale !== nextProps.xScale || props.yScale !== nextProps.yScale || props.width !== nextProps.width || props.height !== nextProps.height || props.theme !== nextProps.theme || props.animate !== nextProps.animate || props.motionDamping !== nextProps.motionDamping || props.motionStiffness !== nextProps.motionStiffness || !_.isEqual(props.top, nextProps.top) || !_.isEqual(props.right, nextProps.right) || !_.isEqual(props.bottom, nextProps.bottom) || !_.isEqual(props.left, nextProps.left);
})(Axes);
...
var isMatchingDef = function isMatchingDef(predicate, node, dataKey) {
    if (predicate === '*') {
        ...
        var data = dataKey ? _.get(node, dataKey) : node;
        return _.isEqual(_.pick(data, Object.keys(predicate)), predicate);
    }
                    ...
                    if (def) {
                        if (patternKeys.includes(def.type)) {
                            if (def.background === 'inherit' || def.color === 'inherit') {
                                var nodeColor = _.get(node, colorKey);
                                ...
                                _.set(node, targetKey, 'url(#' + inheritedId + ')');
                            ...
                            if (allColors.includes('inherit')) {
                                var _nodeColor = _.get(node, colorKey);
                                ...
                                _.set(node, targetKey, 'url(#' + _inheritedId + ')');
